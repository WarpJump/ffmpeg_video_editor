    ...
    try:
        async for message in websocket:
            ...
            if action == "generate_preview_map":
                ...
                video1_path = (
                    segments[0].get("video") if segments else params.get("video1")
                )
                if not video1_path:
                    await send_log(
                        websocket, "Ошибка: Нет видео для обработки!", msg_type="log"
                    )
                    continue

                timeline = await TimelineBuilder(
                    params, "/dev/shm" if os.path.exists("/dev/shm") else "."
                ).build()
                out_dir, base = (
                    params.get("output_dir") or BROWSE_ROOT_OUTPUT
                ), os.path.splitext(os.path.basename(video1_path))[0]
                out_path = os.path.join(
                    out_dir, f"{base}_edited_{str(uuid.uuid4())[:4]}.mkv"
                )
                ...
                await send_log(
                    websocket,
                    f"\nГОТОВО! Финальный файл сохранен по пути:",
                    msg_type="header",
                )
                ...
                root = (
                    BROWSE_ROOT_OUTPUT
                    if data.get("id") == "output_dir"
                    else BROWSE_ROOT_INPUTS
                )
                path = os.path.realpath(
                    os.path.join(root, data.get("path", "/").lstrip("/"))
                )
                entries = [
                ...
                await websocket.send(
                    json.dumps({"action": "path_resolved", "full_path": path})
                )
...
async def http_server_handler(path, request_headers):
    ...
    if path.startswith("/video"):
        ...
        return (
            ...
            open(os.path.join(script_dir, "style.css"), "rb").read(),
        )
    elif path == "/script.js":
        return (
            http.HTTPStatus.OK,
            {"Content-Type": "application/javascript"},
            open(os.path.join(script_dir, "script.js"), "rb").read(),
        )
...
async def main():
    ...
    async with serve(
        websocket_handler, "0.0.0.0", SERVER_PORT, process_request=http_server_handler
    ):
